...
path <-  "/pic/projects/GCAM/CMIP5-CHartin"
...
for (e in ensemble_list){
  ...
  for (v in varlist){
    for (m in modellist){
      ...
      pr_lam <- convert_todf(lam_avg, vname=v)
      pr_lam$ensemble <- e
      
      # write file out, append to same file as you go along
      outfnm <- paste0( OUTPUT_DIR, v, "_", ex, "_", e, ".csv" )
      write.table( pr_lam, file=outfnm, row.names=F, sep=",", col.names=first, append=!first)
      first=FALSE
	rm (d, larea, lfrac, larea.frac, dim_lam, area_lam, lam_avg, pr_lam)
gc()
    }
  }
}
